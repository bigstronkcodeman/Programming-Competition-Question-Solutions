...
string BigInt::toDecimalString()
{
	...
	if (log10base - (int)log10base == 0)
	{
		int maxLen = log10base;
		for (int i = 0; i < base_coefficients.size(); ++i)
		{
			unsigned long long currNumDigits = base_coefficients[i] > 0 ? log10(base_coefficients[i]) + 1 : 1;
...
BigInt distinctSubsequences(string x, string z)
{
	vector<vector<BigInt>> dp(x.size(), vector<BigInt>(z.size(), 0));
	dp[0][0] = z[0] == x[0] ? 1 : 0;
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		BigInt ans = distinctSubsequences(x, z);
		cout << ans.toDecimalString() << endl;
	}
